...
- 工作内容：
  ...
  - 负责产物，基于docker镜像构建，优化多阶段构建，优化dockerfile用好cache机制加速构建，建立规则完成多镜像仓库推送
  ...
  - 持续与工程（所属团队）及算法团队（跨团队）沟通，完成代码仓库的自动化交付设计，实现了单一服务仓库及mono代码仓库的o(1)复杂度交付，配置成本极大降低

  - 负责产品私有化部署设计，实现在客户内网下的全自动化部署
  - 负责制定产物交付流程，对接工程及算法平台团队集成统一交付逻辑，降低交付成本
  - 负责私有化部署效率，实现快速交付，降低实施的边际成本

- 项目经历： 产品私有化部署 [https://www.rcrai.com](https://www.rcrai.com)（录音质检，销售辅助，话术挖掘）
  - 负责建立服务独立代码仓库与多服务混合仓库（monorepo）的代码规则及配置模版，辨识代码提交意图，完成相应的构建及部署
  - 负责研发、测试、基线环境的运维工作，基于ansible设计k8s下的部署流程，并协助完成运行问题排查
  - 基于gitlab-ci达成面向o(1)的自动化集成过程，实现任意服务的产物交付至相应环境部署
...
- 职级职等：M序列 / P4-1
- 职位：AIoT事业群运维部总监 / 高级运维工程师 / 高级研发工程师
- 团队下属： 5人
...
- 工作成果：
  ...
  3. AIUI混合大仓库拆解，一周内完成对cmake构建的学习，并设计出依赖管理方案，对分离出的上百个依赖库及服务完成仓库自动化构建方案，解决仓库臃肿代码合并困难的问题。
  4. AIUI部署流程改进，把原先的声明式构建过程替换，避免构建信息人为提交缺失及错误。
  5. 完成链路跟踪，跟踪服务请求的状态及耗时
  6. 优化日志格式为结构化，并基于字段做相应的报表。告警规则细化并实现抑制，减少无效告警及干扰式告警。
  6. 参与listenAI项目微服务开发，通过ci-pipeline的设计完成芯片固件调优，并上传调优文件供客户下载
- 工作内容：
